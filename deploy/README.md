...
| 用户、分享、询价、报价等业务记录 | NAS 迁移前在隔离副本做双端兼容性合并；NAS 独有记录保留，冲突逐项确认 |

2026-09-12 起，**禁止用本机数据库直接覆盖 NAS 数据库**。此规则不等于已经完成同步，目前没有自动同步任务，也不进行双向 SQLite 文件同步。
实际执行须先确认目标主机和运行中的数据卷，并分别备份本机与 NAS 数据库及图片。随后在隔离副本比较迁移版本、结构、记录与图片引用，输出新增、相同、NAS 独有和冲突清单；无冲突新增项才可自动合并，身份、业务编号、分享码、历史快照、内容和图片冲突必须逐项确认。源库引用的图片缺失时必须中止，不能发布断链数据。
