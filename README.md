...
- 图片目前填写相对于项目根目录的路径；配置目录列表显示缩略图，编辑卡片显示路径。
...
2. 将 `backend/boten.db` 和图片目录做独立、定时、可恢复的备份。
...
- 图片采用路径引用，尚无上传、裁剪、对象存储和失效文件清理流程。
