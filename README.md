...
截至 2026-09-03，项目已经具备可本地运行和联调的完整主流程：
...
- 登录用户可保存多套设备配置，也可独立加入维修工具和设备附件；购物车支持重新编辑、数量调整、多选、混合分享、合并 PDF 和原子批量软删除，分享内容使用不可变双语快照并保留 90 天。
...
- 已建立 65 项自动化测试，覆盖迁移、认证、账号生命周期、客户价格边界、配置覆盖、多设备及混合商品分享、仅工具/附件流程、冻结快照、原子批量归档、分享模糊搜索、目录排序、双语目录、角色权限、V2 目录 CRUD、价格方案、新旧报价兼容、PDF、操作审计、图片接口、安全删除、双语无障碍名称和备份恢复。
- 合并配置 PDF 已增加客户名称、邮箱或手机号、导出时间、品牌页脚和总页数；设备信息与可选配置分区展示，每台设备的可选配置序号从 1 重新开始。
- 后台分享记录支持分页、关键词、状态、设备型号和日期筛选，后台预览不增加客户查看次数，并支持关闭或重新启用未过期分享。
...
用户端不显示或接收任何设备、配置、工具和附件价格；用户端及后台导出的分享配置 PDF 同样不含价格。价格数据继续保存在数据库和历史快照中，仅供管理员/业务员在后台详情、报价编辑和正式报价 PDF 中使用。当前执行状态与验收入口见 [`docs/2026-09-04-project-master-plan.md`](docs/2026-09-04-project-master-plan.md)。
