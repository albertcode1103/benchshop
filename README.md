...
> 当前维护状态以 [2026-09-12 本机维护记录](docs/2026-09-12-maintenance-record.md) 与 [今日合并工作计划](docs/2026-09-12-work-plan.md) 为入口：代码基线为 `09f0131`，数据库迁移为 `20260911_0030`；本轮完整回归为 360 项通过、14 项子测试通过。65 张已审核发布图片保持 Git LFS 暂存、尚未提交。NAS 后续迁移改为先检查并兼容性合并，禁止本机数据库直接覆盖。历史段落保留实施证据，不代表当前发布状态。
