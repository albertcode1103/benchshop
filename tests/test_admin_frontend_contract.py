...
class AdminFrontendContractTests(unittest.TestCase):
    ...
    def test_product_editor_shell_uses_one_consistent_gutter(self) -> None:
        self.assertIn('.product-dialog #product-form > header {', ADMIN_CSS)
        self.assertIn('.product-dialog #product-form > .editor-tabs {', ADMIN_CSS)
        self.assertIn('padding: 6px var(--editor-gutter);', ADMIN_CSS)
        self.assertIn('.product-dialog #product-form > footer {', ADMIN_CSS)
        self.assertIn('class="color-editor-fields"', ADMIN_CATALOG_V2)
        self.assertIn('class="color-action-rail"', ADMIN_CATALOG_V2)
        self.assertIn('.color-action-rail { display: flex;', ADMIN_CSS)
        self.assertIn('color-remove-button" data-remove-color', ADMIN_CATALOG_V2)
        self.assertIn('> [data-content-lang] { grid-column: 1;', ADMIN_CSS)
    ...
    def test_profile_is_a_separate_hierarchical_page(self) -> None:
        ...
        self.assertIn('data-account-panel="my-inquiries"', ACCOUNT_HTML)
        self.assertIn('data-account-panel="my-quotes"', ACCOUNT_HTML)
        self.assertIn('/customer/me/shares?page=', ACCOUNT_JS)
        self.assertIn('function renderProfileDevicePreview(item, index)', ACCOUNT_JS)
        self.assertIn('class="profile-device-option-group"', ACCOUNT_JS)
        self.assertIn('.profile-device-basics > div { display: grid;', ACCOUNT_CSS)
        self.assertIn('/customer/me/inquiries?page=', ACCOUNT_JS)
        self.assertIn('/customer/me/inquiries/${encodeURIComponent(inquiryId)}?lang=', ACCOUNT_JS)
        self.assertIn('/cancel', ACCOUNT_JS)
    ...
    def test_color_editor_uses_bilingual_names_and_image_preview(self) -> None:
        ...
        self.assertIn('id="product-images-editor"', ADMIN_HTML)
        self.assertIn('data-move-product-image', ADMIN_CATALOG_V2)
        self.assertIn('data-remove-product-image', ADMIN_CATALOG_V2)
        self.assertIn('data-preview-product-image', ADMIN_CATALOG_V2)
        self.assertIn('.product-image-row {', ADMIN_CSS)
    ...
    def test_customer_catalog_v2_sales_contact_and_independent_catalog(self) -> None:
        ...
        self.assertIn('const botenIsLocalStaticServer = ["8080", "8081"].includes(window.location.port);', runtime_config)
        ...
        self.assertIn('["8080", "8081"].includes(window.location.port)', runtime_config)
    ...
    def test_cart_header_contains_whole_cart_share_and_pdf_actions(self) -> None:
        ...
        for removed_style in ('.cart-selection-toolbar', '.cart-item-select', '.cart-item.selected', '.cart-batch-actions'):
            self.assertNotIn(removed_style, CUSTOMER_COMPONENTS_CSS)
    ...
    def test_quote_lifecycle_management_exposes_archive_restore_and_history(self) -> None:
        ...
        self.assertIn('id="quote-status-filter"', ADMIN_HTML)
        self.assertIn('function quoteListPath()', ADMIN_JS)
        self.assertIn('new URLSearchParams({ status: state.quoteStatus })', ADMIN_JS)
        ...
        self.assertIn('data-open-inquiry-quote=', ADMIN_JS)
        self.assertIn('async function openInquiryQuote(button)', ADMIN_JS)
        self.assertIn('function inquirySourceSummary(item)', ADMIN_JS)
        self.assertIn('关联分享码', ADMIN_JS)
        self.assertIn('处理时间线', ADMIN_JS)
        self.assertIn('function quoteCustomerContext(quote = {})', ADMIN_JS)
        self.assertIn('recipientUserId: customer.recipientUserId', ADMIN_JS)
        self.assertIn('archivedQuote: "已撤回（仅历史记录）"', ACCOUNT_JS)
        self.assertIn('quote.lifecycle_status === "archived" || quote.prices_hidden', ACCOUNT_JS)
        self.assertIn('quoteVersion: quote.version', ADMIN_JS)
        self.assertIn('version: quoteId ? quoteVersion : null', ADMIN_JS)
        self.assertIn('version: savedQuote.version', ADMIN_JS)
        self.assertIn('const exportAction = status === "archived" ? ""', ADMIN_JS)

    def test_quote_editor_remains_editable_on_phone_viewports(self) -> None:
        self.assertIn('@media (max-width: 600px) {', ADMIN_CSS)
    ...
    def test_admin_sidebar_supports_persistent_desktop_icon_mode(self) -> None:
        self.assertIn('class="sidebar-brand-label">工作台</span>', ADMIN_HTML)
        self.assertIn('id="sidebar-collapse-toggle"', ADMIN_HTML)
        self.assertIn('aria-controls="sidebar"', ADMIN_HTML)
        self.assertIn('const SIDEBAR_COLLAPSED_KEY = "boten_admin_sidebar_collapsed"', ADMIN_JS)
        self.assertIn('function setSidebarCollapsed(collapsed, persist = true)', ADMIN_JS)
        self.assertIn('localStorage.setItem(SIDEBAR_COLLAPSED_KEY', ADMIN_JS)
        self.assertIn('@media (min-width: 1025px)', ADMIN_CSS)
        self.assertIn('.admin-app.sidebar-collapsed { --sidebar-width: 78px; }', ADMIN_CSS)
        self.assertIn('@media (max-width: 1024px)', ADMIN_CSS)
        self.assertIn('.sidebar-collapse-toggle { display: none; }', ADMIN_CSS)

    def test_business_lists_use_compact_fixed_horizontal_layouts(self) -> None:
        self.assertIn('class="table-wrap business-table-wrap inquiry-table-wrap"', ADMIN_HTML)
        self.assertIn('aria-label="询价列表，可左右滑动"', ADMIN_HTML)
        self.assertIn('class="business-table inquiries-business-table"', ADMIN_HTML)
        self.assertIn('.inquiries-business-table { width: 100%; min-width: 1120px; }', ADMIN_CSS)
        self.assertIn('.business-table-wrap { max-width: 100%; overflow-x: auto;', ADMIN_CSS)
        self.assertIn('colspan="7" class="empty">暂无询价记录', ADMIN_JS)
        self.assertIn('colspan="6" class="empty">暂无分享记录', ADMIN_JS)
        self.assertIn('function renderCommerceCountLines(counts)', ADMIN_JS)
        self.assertIn('[Number(counts.device || 0), "台设备"]', ADMIN_JS)
        self.assertIn('[Number(counts.tool || 0), "件工具"]', ADMIN_JS)
        self.assertIn('[Number(counts.accessory || 0), "件附件"]', ADMIN_JS)
        self.assertIn('`<div>${escapeHtml(formatNumber(count))}${label}</div>`', ADMIN_JS)
        self.assertIn('.business-content-counts { display: grid;', ADMIN_CSS)
        self.assertIn('function renderCustomerLines(name, email, phone)', ADMIN_JS)
        self.assertIn('renderCustomerLines(share.sender_name, share.sender_email, share.sender_phone)', ADMIN_JS)
        self.assertIn('renderCustomerLines(quote.customer_name, quote.customer_email, quote.customer_phone)', ADMIN_JS)
        self.assertIn('.business-customer-lines { display: grid;', ADMIN_CSS)
        self.assertIn('id="quote-query"', ADMIN_HTML)
        self.assertIn('function quoteListPath()', ADMIN_JS)
        self.assertIn('quoteSearchTimer = setTimeout', ADMIN_JS)
        self.assertIn('function ensureInquiryDrawer()', ADMIN_JS)
        self.assertIn('className = "share-drawer inquiry-drawer"', ADMIN_JS)
        self.assertIn('function openInquiryDrawer()', ADMIN_JS)
        self.assertIn('function closeInquiryDrawer()', ADMIN_JS)
        self.assertIn('.inquiry-drawer .inquiry-detail-body { overflow: visible; }', ADMIN_CSS)
        self.assertIn('const selectedSourceId = sourceSelect.value;', ADMIN_JS)
        self.assertIn('candidates.some((item) => String(item.id) === selectedSourceId)', ADMIN_JS)
        self.assertIn('const expectedCatalogType = kind === "tool" ? "tools"', ADMIN_JS)
        self.assertIn('.filter(([, value]) => isConfiguredValue(value))', ADMIN_JS)
        self.assertNotIn('["通道", categoryList.some((item) => item.id === "channel") ? singleValue("channel") : "未配置"]', ADMIN_JS)
        self.assertIn('const isConfiguredValue = (value) =>', ADMIN_JS)
        self.assertIn('"未配置", "未选择", "无", "none", "not configured"', ADMIN_JS)
        self.assertIn('renderCustomerLines(share.sender_name, share.sender_email, share.sender_phone)', ADMIN_JS)
    ...
    def test_catalog_pages_add_directly_to_cart_without_pending_summary(self) -> None:
        ...
        self.assertIn('.catalog-product-card.selected .catalog-add-button:disabled {', CUSTOMER_COMPONENTS_CSS)
        self.assertIn('background: #087a55;', CUSTOMER_COMPONENTS_CSS)
        self.assertIn('class="summary-option-code"', CUSTOMER_RENDERER)
        self.assertIn('detailItems,', CUSTOMER_PRICE)
        self.assertIn('id="catalog-drawer-toggle"', CUSTOMER_HTML)
        ...
        self.assertNotIn('class="catalog-product-in-cart"', CUSTOMER_MARKETPLACE)
        self.assertIn('class="catalog-quantity-label"', CUSTOMER_MARKETPLACE)
    ...
    def test_customer_price_boundary_and_unified_top_selector_are_explicit(self) -> None:
        ...
        self.assertGreaterEqual(backend_routes.count('include_prices=False'), 2)
        self.assertIn('render_quote_pdf(result)', backend_routes)
    ...
    def test_inquiry_workflow_is_separate_from_shares_and_exposed_to_staff(self) -> None:
        ...
        self.assertIn('/pdf?lang=', ADMIN_JS)
        self.assertIn('renderQuoteSourceStatus', ADMIN_JS)
        self.assertIn('class="inquiry-share-layout"', ADMIN_JS)
        self.assertIn('class="share-device-summary"', ADMIN_JS)
        self.assertIn('inquiryGroups.devices.map(', ADMIN_JS)
        self.assertIn('.inquiry-share-layout { display: grid;', ADMIN_CSS)
        self.assertIn('class="inquiry-dialog-content"', CUSTOMER_CART)
        self.assertIn('.inquiry-dialog-content { min-height: 0;', CUSTOMER_COMPONENTS_CSS)
        self.assertIn('.inquiry-dialog-card > .cart-confirm-actions { flex: 0 0 auto;', CUSTOMER_COMPONENTS_CSS)
        for marker in ('quote_add_kind', 'quote_add_device', 'quote_add_source', 'data-delete-quote-line', 'reference_price', 'price_overridden'):
            self.assertIn(marker, ADMIN_JS)
        self.assertIn('const isLockedQuoteLine', ADMIN_JS)
        self.assertIn('device_specifications', ADMIN_JS)
        self.assertIn('inquiry.customer_email_snapshot', ADMIN_JS)
        self.assertIn('source_inquiry_id', ADMIN_JS)
        self.assertIn('idempotency_key: deliveryKey', ADMIN_JS)
        self.assertIn('确认发送报价', ADMIN_JS)
        self.assertIn('restoreBusinessFilterState()', ADMIN_JS)
        self.assertIn('function canonicalCommerceItems(', ADMIN_JS)
        self.assertIn('function aggregateCommerceItems(', ADMIN_JS)
        ...
        self.assertNotIn('name="recipient_query"', ADMIN_JS)
        self.assertIn('item.kind === "product" ? "设备基础价格"', ADMIN_JS)
        self.assertIn('event.target.matches(".quote-price-input")', ADMIN_JS)
        self.assertIn('.quote-edit-row:focus-within .quote-item-name', ADMIN_CSS)
        self.assertIn('.quote-commerce-group-title { position: sticky;', ADMIN_CSS)
    ...
    def test_cart_cards_and_destructive_confirmation_use_the_page_design_system(self) -> None:
        ...
        self.assertNotIn('window.confirm(', CUSTOMER_CART)
        self.assertIn('continueEditingTitle', CUSTOMER_CART)
        self.assertNotIn('window.confirm(message)', CUSTOMER_CART)
        self.assertIn('width: min(525px, 80vw)', CUSTOMER_COMPONENTS_CSS)
