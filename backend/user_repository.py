...
PUBLIC_USER_FIELDS = "id, email, phone, phone_country, role, display_name, gender, birth_date, signature, address, enabled, deleted_at, deleted_by, delete_reason, version, created_at, updated_at"
    ...
    with get_connection() as connection:
        row = connection.execute(
            ...
            SELECT u.id, u.email, u.phone, u.phone_country, u.role, u.display_name,
                   u.gender, u.birth_date, u.signature, u.address, u.enabled,
...
def update_user(user_id: str, values: Dict[str, Any], expected_version: Optional[int] = None) -> Optional[Dict[str, Any]]:
    allowed = {"email", "phone", "phone_country", "display_name", "gender", "birth_date", "signature", "address", "role", "password"}
